...
if name == "falling_cabinet_door" then
    thisEntity:ApplyLocalAngularVelocityImpulse(Vector(0, 1200, 0))
    return
end
...
function IsCombineConsoleLocked(console)
    local ents = Entities:FindAllByClassnameWithin("baseanimating", console:GetCenter(), 96)
    for i = 1, #ents do
        local ent = ents[i]
        if vlua.find(ent:GetModelName(), "vr_combine_interface") then
            return ent:GetSequence() == "closed_idle"
        end
    end
    return true
end

function OpenCombineConsoleRack()
    if thisEntity:Attribute_GetIntValue("rack_pulled", 0) == 1 then
        return
    end

    local console = Entities:FindByClassnameNearest("item_combine_console", thisEntity:GetCenter(), 128)
    if not console or IsCombineConsoleLocked(console) then
        return
    end

    thisEntity:Attribute_SetIntValue("rack_pulled", 1)
    DoEntFireByInstanceHandle(thisEntity, "DisableReturnToCompletion", "", 0, nil, nil)

    if class == "item_hlvr_combine_console_rack" then
        DoEntFireByInstanceHandle(console, "RackOpening", "", 0, thisEntity, thisEntity)

        local count = thisEntity:GetCycle()
        thisEntity:SetThink(function()
            count = count + 0.01
            DoEntFireByInstanceHandle(thisEntity, "SetCompletionValue", "" .. math.min(count, 1), 0, nil, nil)
            if count < 1 then
                return 0
            end
            if ShowHoldInteractTutorial and Entities:FindByName(nil, "text_holdinteract") then
                ShowHoldInteractTutorial()
            end
            return nil
        end, "OpenCombineConsoleRack", 0)
        return
    end

    DoEntFireByInstanceHandle(thisEntity, "PlayAnimation", thisEntity:GetSequence(), 0, nil, nil)

    if console:Attribute_GetIntValue("internals_extended", 0) == 0 then
        console:Attribute_SetIntValue("internals_extended", 1)
        DoEntFireByInstanceHandle(console, "InternalsExtended", "", 0, thisEntity, thisEntity)
...
if not vlua.find(model, "doorhandle") and name ~= "russell_entry_window" and name ~= "larry_ladder" and name ~= "@pod_shell" and name ~= "589_panel_switch" and name ~= "tc_door_control" and (class == "item_health_station_charger" or (class == "prop_animinteractable" and model ~= "models/props_combine/combine_consoles/vr_console_rack_1.vmdl" and (not vlua.find(name, "elev_anim_door") or (vlua.find(name, "elev_anim_door") and thisEntity:Attribute_GetIntValue("toggle", 0) == 1 and thisEntity:GetVelocity() == Vector(0, 0, 0))) and not vlua.find(name, "5628_2901_barricade_door"))) and not (map == "a4_c17_parking_garage" and name == "door_reset" and player:Attribute_GetIntValue("circuit_" .. map .. "_toner_junction_5_completed", 0) == 0 and not NoVR.TonerPathElectrified("toner_path_8")) and thisEntity:Attribute_GetIntValue("used", 0) == 0 then
    ...
    thisEntity:SetThink(function()
        ...
        if not (map == "a3_distillery" and name == "verticaldoor_wheel") and count >= 1 or count >= 10 then
            ...
            if name ~= "barricade_door_hook" then
                ...
                if map == "a4_c17_parking_garage" and name == "toner_sliding_ladder" then
                    SendToConsole("ent_fire 18035_sliding_ladder Enable")
                end
                if map == "a3_distillery" and name == "cellar_ladder" then
                    SendToConsole("ent_fire pull_ladder_volume Enable")
                end

                if vlua.find(name, "slide_train_door") then
                    thisEntity:FireOutput("OnCompletionB", nil, nil, nil, 0)
                    thisEntity:FireOutput("OnCompletionD", nil, nil, nil, 0)
                end
...
elseif (name == "barricade_door_hook" and player:Attribute_GetIntValue("locked_jeff_in_freezer", 0) == 0) or (name == "589_panel_switch" and NoVR.TonerPathElectrified("589_path_11")) or name == "5628_2901_barricade_door_hook" or name == "tc_door_control" or (vlua.find(name, "elev_anim_door") and thisEntity:Attribute_GetIntValue("toggle", 0) == 0 and thisEntity:GetVelocity() == Vector(0, 0, 0)) then
...
if name == "205_2724_hingecam" then
    NoVR.BorrowKey("j", "novr_leavehingecam")
    NoVR.ShowHint("hingecam")
...
if name == "glove_dispenser_brush" and thisEntity:Attribute_GetIntValue("used", 0) == 0 then
    ...
    if NoVR_PublishGravityGloves then NoVR_PublishGravityGloves() end
end
...
if name == "combine_gun_mechanical" then
    NoVR.BorrowKey("j", "novr_leavecombinegun")
    NoVR.ShowHint("combinegun")
    if thisEntity:Attribute_GetIntValue("used", 0) == 0 then
...
if class == "item_hlvr_combine_console_rack" or (class == "prop_animinteractable" and vlua.find(model, "combine_consoles/vr_console_rack")) then
    OpenCombineConsoleRack()
end
...
if name == "traincar_01_hatch" and thisEntity:Attribute_GetIntValue("used", 0) == 0 then
    thisEntity:Attribute_SetIntValue("used", 1)
    local ent = Entities:FindByName(nil, "traincar_01_hackplug")
    ent:Attribute_SetIntValue("used", 1)
...
if (model == "models/props/hazmat/respirator_01b.vmdl" or model == "models/props/hazmat/respirator_01a.vmdl") and name ~= "respirator" then
    ...
    SendToConsole("ent_fire npc_barnacle SetRelationship \"player D_NU 99\"")
    if Convars:GetInt("hidehud") ~= 104 and Convars:GetInt("hidehud") ~= 9 then
        SendToConsole("r_drawviewmodel 1")
    end
    ...
    if thisEntity:Attribute_GetIntValue("no_pick_up", 0) == 0 then
        ...
        if class == "item_hlvr_health_station_vial" then
            ...
            if entcharger ~= nil then
                if entcharger:GetSequence() ~= "idle_deployed" and entcharger:GetSequence() ~= "prepare_inject" and entcharger:GetSequence() ~= "idle_retracted" then
