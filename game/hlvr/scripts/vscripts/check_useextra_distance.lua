...
if eyetrace.hit then
    ...
    if VectorDistance(startVector, eyetrace.pos) <= cvar_getf("player_use_radius")
        and useRoutine == 0
        and (eyetrace.enthit == thisEntity or vlua.find(name, "door_hack"))
        and IsValidEntity(thisEntity) then
        DoEntFireByInstanceHandle(thisEntity, "RunScriptFile", "useextra", 0, nil, nil)
    end
end
